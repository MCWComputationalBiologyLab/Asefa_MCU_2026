...
beta_MCUx  = 0.5*(1 + log(E_dPhi)/dPhi);         % Symmetry factor (dimensionless)
exp_e = exp(+2*beta_MCUx*dPhi);                  % Forward exponential term
exp_x = exp(-2*(1-beta_MCUx)*dPhi);              % Reverse exponential term
